...
int main(int argc, char *argv[])
{
    ...
    TDCS_SLAM::System SLAM(argv[1],argv[2],TDCS_SLAM::System::IMU_MONOCULAR, true);
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        // Main loop
        cv::Mat im;
        vector<TDCS_SLAM::IMU::Point> vImuMeas;
        proccIm = 0;
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            if(ni>0)
            {
                ...
                {
                    vImuMeas.push_back(TDCS_SLAM::IMU::Point(vAcc[seq][first_imu[seq]].x,vAcc[seq][first_imu[seq]].y,vAcc[seq][first_imu[seq]].z,
    ...
    if (bFileName)
    {
        ...
        #ifdef REGISTER_TIMES
        const string time_file = string(argv[argc-1]) + ".txt";        
        SLAM.printTimeInformation(time_file);
        #endif
    }
    else
    {
