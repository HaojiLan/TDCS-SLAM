...
public:
  ...
  virtual void transform(const std::vector<TDescriptor>& features,
    BowVector &v, FeatureVector &fv, int levelsup,const vector<bool> &computeflag) const;
...
//  fv.clear();
  ...
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    ...
    //particularly for initial keyframe computation
    if(computeflag.size() == 1){
      ...
      for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
      {
        if(computeflag[fit-features.begin()]){
          WordId id;
          NodeId nid;
          WordValue w; 
          // w is the idf value if TF_IDF, 1 if TF
