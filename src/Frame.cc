     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mTimeORB_Ext = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(time_EndExtORB - time_StartExtORB).count();
#endif
 //   cout<<"extract ORB time: "<<mTimeORB_Ext<<endl;
    if(!mvKeys.empty()){
        N = mvKeys.size();
        UndistortKeyPoints();
    }
    else
        N = mpORBextractorLeft->Getnfeatures()+50;
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1)
{
    ...
    bool InitialFrame;
//    if (mpORBextractorLeft->nfeatures=1500)
 //        InitialFrame = mpORBextractorLeft->Getnfeatures() > 2000 ? true:false;
  //  else
    InitialFrame = mpORBextractorLeft->Getnfeatures() > 3000 ? true:false;
    monoLeft = (*mpORBextractorLeft)(im,cv::Mat(), mvKeys, mDescriptors, vLapping, InitialFrame);
}
...
void Frame::ComputeBoW(const vector<bool> &ComputeFlag)
{
    if(mBowVec.empty())
    {
        vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
        mpORBvocabulary->transform(vCurrentDesc,mBowVec,mFeatVec,4, ComputeFlag);
    }
}
