    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    mptLocalMapping = new thread(&TDCS_SLAM::LocalMapping::Run,mpLocalMapper);
    mpLocalMapper->mInitFr = initFr;
     mpLocalMapper->mbCreateMP=true;
    if(settings_)
    ...
    mptLoopClosing = new thread(&TDCS_SLAM::LoopClosing::Run, mpLoopCloser);
...
#ifdef REGISTER_TIMES
void System::printTimeInformation(const string& fileName){
    mpTracker->TrackStats2File(fileName);
}
#endif
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    list<TDCS_SLAM::KeyFrame*>::iterator lRit = mpTracker->mlpReferences.begin();
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        f << *lT << "e-09 " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
    f.close();
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        f << pKF->mTimeStamp << "e-09"<<setprecision(7) << " " << t(0) << " " << t(1) << " " << t(2)
