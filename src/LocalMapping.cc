            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        {
                            float th1, th2;
                            int  nfeature = mpTracker->Getnfeature();
                            if(mThFarPoints|| nfeature>1250){
                                th1=10.f;
                                th2=0.02;
                            }
                            else{
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // exclude Initial KeyFrame
    if(!mpCurrentKeyFrame->InitialKFflag){
        int  nfeature = mpTracker->Getnfeature();
        ORBextractor* extractor = new ORBextractor(nfeature, 1.2, 8, 20, 7);
        extractor->FullComputeDescritor(mpCurrentKeyFrame->mallKeypoint, mpCurrentKeyFrame->mvWorkingMat, 
                                        mpCurrentKeyFrame->mDescriptors, mpCurrentKeyFrame->mbCompute);

        mpCurrentKeyFrame->mallKeypoint.clear();
        mpCurrentKeyFrame->mvWorkingMat.clear();

        delete extractor;
        // Compute Bags of Words structures
        mpCurrentKeyFrame->ComputeBoW(mpCurrentKeyFrame->mbCompute);

        mpCurrentKeyFrame->mbCompute.clear();    
    }
...
void LocalMapping::MapPointCulling()
{
    ...
    while(lit!=mlpRecentAddedMapPoints.end())
    {
        ...
        else if(((int)nCurrentKFid-(int)pMP->mnFirstKFid)>=3 && pMP->Observations()<=cnThObs)
        {
            pMP->SetBadFlag();
            lit = mlpRecentAddedMapPoints.erase(lit);
        }
...
void LocalMapping::CreateNewMapPoints()
{
  // int DB_count=0;
    ...
    {
        if(i>0 && CheckNewKeyFrames() && !mbFarPoints)
            return;
  ...
  //  cout<<"creat new mappoints, num is:"<<DB_count<<endl;    
}
        ...
        if(nRedundantObservations>redundant_th*nMPs)
        {
            if (mbInertial)
            {
                ...
                if(pKF->mPrevKF && pKF->mNextKF)
                {
                    ...
                    // after IniertialBA2(), culling KF created even without any movement.
                    bool dist_flag = ((pKF->GetImuPosition()-pKF->mPrevKF->GetImuPosition()).norm()<0.02);
                    if((bInitImu && (((pKF->mnId<last_ID) && (t<3)) || dist_flag)) || (t<0.5))
                    {                                                
                    ...
                    else if(!mpCurrentKeyFrame->GetMap()->GetIniertialBA2() && dist_flag && (t<3))
                    {                      
                        pKF->mNextKF->mpImuPreintegrated->MergePrevious(pKF->mpImuPreintegrated);
                        pKF->mNextKF->mPrevKF = pKF->mPrevKF;
                        pKF->mPrevKF->mNextKF = pKF->mNextKF;
