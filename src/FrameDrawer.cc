...
//cv::Mat FrameDrawer::DrawFrame(int & number, float imageScale)
cv::Mat FrameDrawer::DrawFrame( float imageScale)
{
   ...
   // cv::Scalar standardColor(0,0,255);
            ...
            if(vbVO[i] || vbMap[i])
            {
                ...
                if(vbMap[i])
                {
                    cv::rectangle(im,pt1,pt2,standardColor);
                     cv::circle(im,point,1,standardColor,-1);  
                    // if(mflagKF)
                    //     cv::rectangle(im,pt1,pt2,standardColor);
                    // else             
                    //     cv::circle(im,point,4,standardColor,-1);
                    mnTracked++;
                }
                else // This is match to a "visual odometry" MapPoint created in the last frame
                {
                    cv::rectangle(im,pt1,pt2,odometryColor);
                    cv::circle(im,point,4,odometryColor,-1);
                    mnTrackedVO++;
                }
            }
        }
    ...
    /*save image */
    // string rootPath = "/home/hj/FAST_SLAM/result/image_resut_TDCS/";
    // string imageName = to_string(number);
    // string FilePath = rootPath+imageName+".jpg";
    // cv::Mat s_image;
    // im.convertTo(s_image, -1, 1, 20); // brighten image.
    // cv::imwrite(FilePath , s_image);   
...
//void FrameDrawer::Update(Tracking *pTracker, const bool & flagKF)
void FrameDrawer::Update(Tracking *pTracker)
{
    unique_lock<mutex> lock(mMutex);
    pTracker->mImGray.copyTo(mIm);

    mvCurrentKeys=pTracker->mCurrentFrame.mvKeysUn;
