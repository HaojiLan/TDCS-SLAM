...
/*
Tracking::Tracking(const cv::Mat &img1_, const cv::Mat &img2_, std::vector<cv::KeyPoint> &kp1_, std::vector<cv::KeyPoint> &kp2_, std::vector<bool> &success_) :
...
#ifdef REGISTER_TIMES
    void Tracking::clearTimeInformation() {
...
void Tracking::TrackStats2File(const string& fileName)
{
    ofstream f;
    ...
    //f << "#Image Rect[ms], Image Resize[ms], ORB ext[ms], Stereo match[ms], IMU preint[ms], Pose pred[ms], LM track[ms],  Total[ms]" << endl;
    f << "#ORB ext[ms], OF track[ms], Pose pred[ms], LM track[ms],  Total[ms]" << endl;
...
*/
        f <<  vdORBExtract_ms[i] << "," <<vdOFTrack_ms[i]<< ","
          << vdPosePred_ms[i] <<  "," << vdLMTrack_ms[i] << ","  << vdTrackTotal_ms[i] << endl;
    }
...
*/
    ...
    average = calcAverage(vdOFTrack_ms);
    deviation = calcDeviation(vdOFTrack_ms, average);
    std::cout << "opticalFlow: " << average << "$\\pm$" << deviation << std::endl;
    f << "opticalFLOW: " << average << "$\\pm$" << deviation << std::endl;

    average = calcAverage(vdNewKF_ms);
    deviation = calcDeviation(vdNewKF_ms, average);
    std::cout << "Creat newKF: " << average << "$\\pm$" << deviation << std::endl;
    f << "Creat newKF: " << average << "$\\pm$" << deviation << std::endl;
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if(Getnfeature()<1250){
        if(!mpAtlas->GetCurrentMap()->GetIniertialBA2()){
            cv::Ptr<CLAHE> clahe = cv::createCLAHE(3, cv::Size(8,8));
...
bool Tracking::OFTracking(){
    vector<cv::Point2f> pt1;   
    //if last frame is a keyframe, update keypoint tracked by opticalflow.      
    if(mnLastKeyFrameId==mLastFrame.mnId ){
        auto first = mLastFrame.mvpMapPoints.begin();
        auto end = mLastFrame.mvpMapPoints.end();
        while(first!=end){
            if(*first){
                int index = first-mLastFrame.mvpMapPoints.begin();
                mvMapPoints.emplace_back(*first);       // mappoints
                FeatureIndex.emplace_back(index);        // corresponding feature index in the last keyframe.               
            }                                                  
            first++; 
        }

        for(auto ptr : FeatureIndex){
            pt1.emplace_back(mLastFrame.mvKeysUn[ptr].pt);                    //  position of keypoint
            last_oct.emplace_back(mLastFrame.mvKeysUn[ptr].octave);   // record corresponding level 
        }

    }               
    else
        pt1=pt2;    //the  last frame is not a keyframe
   
    pt2.clear();

    vector<uchar> status;
    vector<float> error;  
    const Mat pyr1 = LK_image.at(0);
    const Mat pyr2 = LK_image.at(1);

    int LK_num = pt1.size();
    if(LK_num<4){
    //    cout<<"last frame only has: "<<LK_num<<" mappoints ,  abandon LK track! "<<endl;
        return false;
    }

    calcOpticalFlowPyrLK(pyr1, pyr2, pt1, pt2, status, error);

    //remove lost tracking point           
    for(int i=pt1.size()-1; i>=0;i--){
        if(!status[i]){
            LK_num--;
            pt2.erase(pt2.begin()+i);
            mvMapPoints.erase(mvMapPoints.begin()+i);    
            FeatureIndex.erase(FeatureIndex.begin()+i);   
            last_oct.erase(last_oct.begin()+i);                           
        }
    }
...
void Tracking::RemoveOFpoints(){
   // int bad_count = 0;
    int LK_num = pt2.size()-1;
    //remove lost tracking point           
    for(int i=LK_num; i>=0;i--){
 ...
 //   cout<<"optimize bad points: "<<bad_count<<endl;        
}
...
void Tracking::Track()
{
    ...
    // update currentFrame and lastFrame image in LK tracking.
    if(LK_image.size()>1)
        LK_image.erase(LK_image.begin());
    
    LK_image.emplace_back(mCurrentFrame.mpORBextractorLeft->mvImagePyramid[0].clone());
...
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point StartLK_Tracking = std::chrono::steady_clock::now();
#endif       

     LK_flag = true;
    // abandon optical flow tracking if last frame does not have enough mappoints when mState==RecentLost!
    if(mState==RECENTLY_LOST && mnLastKeyFrameId!=mLastFrame.mnId){
      //  cout<<"don't create kf too frequently!"<<endl;
        LK_flag=false;
    }
    else
        LK_flag =OFTracking();
...
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point EndLK_Tracking = std::chrono::steady_clock::now();
    double timeLK_Tracking = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(EndLK_Tracking - StartLK_Tracking).count();
    vdOFTrack_ms.push_back(timeLK_Tracking);
#endif               
            if(mState==OK)
            {
                    ...
                    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD))
                    {
                        if(pCurrentMap->isImuInitialized()){
                            ...
                            if(LK_flag){
                                ...
                                //UpdateLastFrame();
                                mCurrentFrame.mvpMapPoints = mvMapPoints;
                                mCurrentFrame.mvKeysUn.resize(mvMapPoints.size());
                                // replace with the points tracked by opticalflow successfully!
                                for(auto ptr: pt2){
                                    mCurrentFrame.mvKeysUn[nmatches] = KeyPoint(ptr, 1.f);
                                    mCurrentFrame.mvKeysUn[nmatches].octave =last_oct[nmatches];        
                                    nmatches++;
                                }
                                mCurrentFrame.mvpMapPoints.insert(mCurrentFrame.mvpMapPoints.end(), mCurrentFrame.N-nmatches, static_cast<MapPoint*>(NULL)); 
                        ...
                        if (mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost) //time_recently_lost
                        {
                            mState = LOST;
        ...
        {        
            ...
            if(!LK_flag) {
                mCurrentFrame.mvpMapPoints.clear();
                mCurrentFrame.mvpMapPoints.insert(mCurrentFrame.mvpMapPoints.end(), mCurrentFrame.N, static_cast<MapPoint*>(NULL)); 
            }              
            if(bOK)
                bOK = TrackLocalMap();
        ...
        {
            // update LK tracking mappoints if possible.
            FeatureIndex.clear();
            mvMapPoints.clear();
            last_oct.clear();            
      ...
      //  mpFrameDrawer->Update(this, false);
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
          ...
          //cout<<"timeNewKF: "<<timeNewKF<<endl;
            ...
            //关键帧显示    
            // if(bNeedKF){
            //     mpFrameDrawer->Update(this, bNeedKF );
            // }
        }
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    vector<bool> InitialCompute{true};
    pKFini->ComputeBoW(InitialCompute);
    pKFcur->ComputeBoW(InitialCompute);
    ...
    mpLocalMapper->inliers =  mpAtlas->MapPointsInMap();
    ...
    pKFini->InitialKFflag = true;
    pKFcur->InitialKFflag = true; 
    mpLocalMapper->InsertKeyFrame(pKFini);
...
bool Tracking::TrackReferenceKeyFrame()
{
  //  cout<<"TrackReferenceKeyFrame()"<<endl;
    int nmatches = 0; // mappoints count
    //set currentframe mappoint and their feature information, prepare for optimizer in next step.
    mCurrentFrame.mvpMapPoints.clear();
    mCurrentFrame.mvpMapPoints = mvMapPoints;
    mCurrentFrame.mvKeysUn.resize(mvMapPoints.size());
    for(auto ptr: pt2){
...
bool Tracking::TrackLocalMap()
{
    ...
    //cout<<"after tracklocalmap, mnMatchesInliers= "<<mnMatchesInliers<<endl;     
    ...
    if((mnMatchesInliers>5)&&(mState==RECENTLY_LOST))
        return true;

    if(mSensor == System::IMU_MONOCULAR)
    {
    //    if((mnMatchesInliers<8 && mpAtlas->isImuInitialized())||(mnMatchesInliers<5 && !mpAtlas->isImuInitialized()) )
     //    if((mnMatchesInliers<10&& mpAtlas->GetCurrentMap()->GetIniertialBA2() )|| (mnMatchesInliers<5 && !mpAtlas->GetCurrentMap()->GetIniertialBA2()))
...
bool Tracking::NeedNewKeyFrame_new()
{

    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.3)
            return true;
   ...
   if(mnMatchesInliers<th)
    ...
    // Thresholds
    float thRefRatio =0.4f;
    if(!mpAtlas->GetCurrentMap()->GetIniertialBA2())
        thRefRatio =0.75f;  //0.6f
  ...
  //if ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && (mSensor == System::IMU_MONOCULAR))
    if ( (mState==RECENTLY_LOST) && (mSensor == System::IMU_MONOCULAR)) 
        c4=true;
    else
        c4=false;
    ...
    if(((c1a||c1b) && c2)||c3 ||c4)
    {
    //   cout<<"c2: "<<c2<<" mnMatchesInliers: "<< mnMatchesInliers<<" nRefMatches: "<<nRefMatches<<endl;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mSensor==System::IMU_MONOCULAR)
    {
        if(mnMatchesInliers>50) // Points tracked from the local map
            thRefRatio = 0.6f;
        else
            thRefRatio = 0.8f;
    }
...
int Tracking::SearchKeyFeature(Frame &CurrentFrame, KeyFrame* LastKeyFrame, vector<bool> &bCompute){
    int nmatches = 0;
    bCompute.clear();
    bCompute.insert(bCompute.begin(), CurrentFrame.N, false);
    // Step 2 计算当前帧和前一帧的平移向量
    //当前帧的相机位姿
    const Sophus::SE3f Tcw = CurrentFrame.GetPose();
    const Eigen::Vector3f twc = Tcw.inverse().translation();

    const Sophus::SE3f Tlw = LastKeyFrame->GetPose();
    const Eigen::Vector3f tlc = Tlw * twc; 

    //  Step 3 对于前一帧的每一个地图点，通过相机投影模型，得到投影到当前帧的像素坐标
    for(int i=0; i<LastKeyFrame->N; i++)
    {
        MapPoint* pMP = LastKeyFrame->mvpMapPoints[i];
        ...
        {                      
                ...
                if(uv(1)<CurrentFrame.mnMinY || uv(1)>CurrentFrame.mnMaxY)
                    continue;
                // 认为投影前后地图点的尺度信息不变
                int nLastOctave = LastKeyFrame->mvKeys[i].octave;
                int th;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    last_oct.clear();    
    mvMapPoints.clear();
    //Extract features when create KeyFrame!
    bool boutflag=false;
    
    if(mpLocalMapper->mbFarPoints && mnMatchesInliers<50)
        boutflag=true;     

    mCurrentFrame.mpORBextractorLeft->KeyFrameFeatures(mCurrentFrame.mvKeys, boutflag);

    // abandon create new KF if there does not exist enough features.
    if(mCurrentFrame.mvKeys.size()<50){
        cout<<"mCurrentFrame.mvKeys.size()<50, don't create KF!"<<endl;
        return;
    }

    mCurrentFrame.N = mCurrentFrame.mvKeys.size();
    mCurrentFrame.UndistortKeyPoints();  
    mCurrentFrame.AssignFeaturesToGrid();

    if(mpLocalMapper->mbFarPoints && mnMatchesInliers<35){
        mbCompute.clear();
        mbCompute.insert(mbCompute.begin(), mCurrentFrame.N, true);
        mCurrentFrame.mpORBextractorLeft->KeyFrameDescritors(mCurrentFrame.mDescriptors, mbCompute);
    }else{
        int compute_num = SearchKeyFeature(mCurrentFrame, mpLastKeyFrame, mbCompute);  
        mCurrentFrame.mpORBextractorLeft->KeyFrameDescritors(mCurrentFrame.mDescriptors, mbCompute);    
    }

  // cout<<"==== New KeyFrame!  ====="<<endl;
    ORBmatcher matcher(0.7, true);      
    mCurrentFrame.ComputeBoW(mbCompute);
    int nmatches = matcher.SearchByBoW(mpLastKeyFrame,mCurrentFrame,mCurrentFrame.mvpMapPoints);     
 //   cout << "Match mapponit wit last KF: "<< nmatches <<endl; 
    ...
    pKF->mallKeypoint = mCurrentFrame.mpORBextractorLeft->allKeypoint;
    pKF->mvWorkingMat = mCurrentFrame.mpORBextractorLeft->mWorkingMat;  
    pKF->mbCompute = mbCompute;
    ...
    mpLocalMapper->inliers = mnMatchesInliers;
...
int Tracking::SearchLocalPoints()
{
    ...
    //    cout<<"add local mappoint num:"<< matches <<endl;
    //    cout<<"*****************************"<<endl;
        return matches;
    }
}
...
bool Tracking::Relocalization()
{
    ...
    vector<bool> RelocalCompute(mCurrentFrame.N, true);
    mCurrentFrame.ComputeBoW(RelocalCompute);
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    unsigned int index = mnFirstFrameId;
    list<TDCS_SLAM::KeyFrame*>::iterator lRit = mlpReferences.begin();
