                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    vector<TDCS_SLAM::EdgeSE3ProjectXYZ*> vpEdgesMono;
    vpEdgesMono.reserve(nExpectedSize);

    vector<TDCS_SLAM::EdgeSE3ProjectXYZToBody*> vpEdgesBody;
       ...
       const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(leftIndex != -1 && pKF->mvuRight[get<0>(mit->second)]<0)
            {
                ...
                TDCS_SLAM::EdgeSE3ProjectXYZ* e = new TDCS_SLAM::EdgeSE3ProjectXYZ();
            ...
            if(pKF->mpCamera2){
                ...
                if(rightIndex != -1 && rightIndex < pKF->mvKeysRight.size()){
                    ...
                    TDCS_SLAM::EdgeSE3ProjectXYZToBody *e = new TDCS_SLAM::EdgeSE3ProjectXYZToBody();
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        else
        {
            ...
            if(dist > 1)
            {
                ...
                {
                    TDCS_SLAM::EdgeSE3ProjectXYZ* e = vpEdgesMono[i2];
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    vector<TDCS_SLAM::EdgeSE3ProjectXYZOnlyPose*> vpEdgesMono;
    vector<TDCS_SLAM::EdgeSE3ProjectXYZOnlyPoseToBody *> vpEdgesMono_FHR;
        ...
        if(pMP)
        {
            ...
            if(!pFrame->mpCamera2){
                ...
                if(pFrame->mvuRight[i]<0)
                {
                    ...
                    TDCS_SLAM::EdgeSE3ProjectXYZOnlyPose* e = new TDCS_SLAM::EdgeSE3ProjectXYZOnlyPose();
            ...
            //SLAM with respect a rigid body
                ...
                if (i < pFrame->Nleft) {    //Left camera observation
                    ...
                    TDCS_SLAM::EdgeSE3ProjectXYZOnlyPoseToBody *e = new TDCS_SLAM::EdgeSE3ProjectXYZOnlyPoseToBody();
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        std::chrono::steady_clock::time_point time_Start_tracking = std::chrono::steady_clock::now();        
        optimizer.optimize(its[it]);
        std::chrono::steady_clock::time_point time_End_tracking = std::chrono::steady_clock::now();
        double time_whole_tracking = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(time_End_tracking - time_Start_tracking).count();
//        cout<<"optimize_time:"<< time_whole_tracking <<"ms"<<endl;
        
        nBad=0;
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            TDCS_SLAM::EdgeSE3ProjectXYZOnlyPose* e = vpEdgesMono[i];
        ...
        {
            TDCS_SLAM::EdgeSE3ProjectXYZOnlyPoseToBody* e = vpEdgesMono_FHR[i];
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        TDCS_SLAM::EdgeSE3ProjectXYZ* e = vpEdgesMono[i];
    ...
    {
        TDCS_SLAM::EdgeSE3ProjectXYZToBody* e = vpEdgesBody[i];
    ...
    if(vToErase.size()<(num_edges*0.8)){
        if(!vToErase.empty())
        {
                            ...
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    ...
    // Set Sim3 vertex
    TDCS_SLAM::VertexSim3Expmap * vSim3 = new TDCS_SLAM::VertexSim3Expmap();
    vSim3->_fix_scale=bFixScale;
    ...
    vector<TDCS_SLAM::EdgeSim3ProjectXYZ*> vpEdges12;
    vector<TDCS_SLAM::EdgeInverseSim3ProjectXYZ*> vpEdges21;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        TDCS_SLAM::EdgeSim3ProjectXYZ* e12 = new TDCS_SLAM::EdgeSim3ProjectXYZ();
        ...
        TDCS_SLAM::EdgeInverseSim3ProjectXYZ* e21 = new TDCS_SLAM::EdgeInverseSim3ProjectXYZ();
    ...
    {
        TDCS_SLAM::EdgeSim3ProjectXYZ* e12 = vpEdges12[i];
        TDCS_SLAM::EdgeInverseSim3ProjectXYZ* e21 = vpEdges21[i];
        if(!e12 || !e21)
            continue;
        ...
        if(e12->chi2()>th2 || e21->chi2()>th2)
        {
            ...
            vpEdges12[i]=static_cast<TDCS_SLAM::EdgeSim3ProjectXYZ*>(NULL);
            vpEdges21[i]=static_cast<TDCS_SLAM::EdgeInverseSim3ProjectXYZ*>(NULL);
            nBad++;
